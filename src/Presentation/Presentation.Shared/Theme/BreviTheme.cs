namespace Presentation.Shared.Theme;

public class BreviTheme
{
    public static readonly MudTheme Themetheme = new()
    {
        PaletteLight = new PaletteLight()
        {
            Black = "#594ae2ff",
            White = "rgba(255,255,255,1)",
            Primary = "#fd9600ff",
            PrimaryContrastText = "#fdf8f8ff",
            Secondary = "#333333ff",
            SecondaryContrastText = "rgba(255,255,255,1)",
            Tertiary = "#607d8bff",
            TertiaryContrastText = "rgba(255,255,255,1)",
            Info = "rgba(33,150,243,1)",
            InfoContrastText = "rgba(255,255,255,1)",
            Success = "rgba(0,200,83,1)",
            SuccessContrastText = "rgba(255,255,255,1)",
            Warning = "#ffb300ff",
            WarningContrastText = "rgba(255,255,255,1)",
            Error = "rgba(244,67,54,1)",
            ErrorContrastText = "rgba(255,255,255,1)",
            Dark = "rgba(66,66,66,1)",
            DarkContrastText = "rgba(255,255,255,1)",
            TextPrimary = "rgba(66,66,66,1)",
            TextSecondary = "rgba(0,0,0,0.5372549019607843)",
            TextDisabled = "rgba(0,0,0,0.3764705882352941)",
            ActionDefault = "rgba(0,0,0,0.5372549019607843)",
            ActionDisabled = "rgba(0,0,0,0.25882352941176473)",
            ActionDisabledBackground = "rgba(0,0,0,0.11764705882352941)",
            Background = "#e6e6e6ff",
            BackgroundGray = "#f5f5f5ff",
            Surface = "rgba(255,255,255,1)",
            DrawerBackground = "#ffffffff",
            DrawerText = "rgba(66,66,66,1)",
            DrawerIcon = "#616161ff",
            AppbarBackground = "#333333ff",
            AppbarText = "rgba(255,255,255,1)",
            LinesDefault = "rgba(0,0,0,0.11764705882352941)",
            LinesInputs = "rgba(189,189,189,1)",
            TableLines = "rgba(224,224,224,1)",
            TableStriped = "rgba(0,0,0,0.0196078431372549)",
            TableHover = "rgba(0,0,0,0.0392156862745098)",
            Divider = "rgba(224,224,224,1)",
            DividerLight = "rgba(0,0,0,0.8)",
            PrimaryDarken = "#cc7800ff",
            PrimaryLighten = "#ffb74dff",
            SecondaryDarken = "#1a1a1aff",
            SecondaryLighten = "#4d4d4dff",
            TertiaryDarken = "#455a64ff",
            TertiaryLighten = "#90a4aeff",
            InfoDarken = "rgb(12,128,223)",
            InfoLighten = "rgb(71,167,245)",
            SuccessDarken = "rgb(0,163,68)",
            SuccessLighten = "rgb(0,235,98)",
            WarningDarken = "#ffa000ff",
            WarningLighten = "#ffe082ff",
            ErrorDarken = "rgb(242,28,13)",
            ErrorLighten = "rgb(246,96,85)",
            DarkDarken = "rgb(46,46,46)",
            DarkLighten = "rgb(87,87,87)",
            HoverOpacity = 0.06,
            RippleOpacity = 0.1,
            RippleOpacitySecondary = 0.2,
            GrayDefault = "#9E9E9E",
            GrayLight = "#BDBDBD",
            GrayLighter = "#E0E0E0",
            GrayDark = "#757575",
            GrayDarker = "#616161",
            OverlayDark = "rgba(33,33,33,0.4980392156862745)",
            OverlayLight = "#ffffff7f",
        },
        PaletteDark = new PaletteDark()
        {
            Black = "#27272fff",
            Primary = "#fd9600ff",
            Secondary = "#ffffffff",
            SecondaryContrastText = "#27272fff",
            Info = "rgba(50,153,255,1)",
            Success = "rgba(11,186,131,1)",
            Warning = "rgba(255,168,0,1)",
            Error = "rgba(246,78,98,1)",
            Dark = "#27272fff",
            TextPrimary = "rgba(255,255,255,0.6980392156862745)",
            TextSecondary = "#ffffff7f",
            TextDisabled = "rgba(255,255,255,0.2)",
            ActionDefault = "rgba(173,173,177,1)",
            ActionDisabled = "rgba(255,255,255,0.25882352941176473)",
            ActionDisabledBackground = "rgba(255,255,255,0.11764705882352941)",
            Background = "#333333ff",
            BackgroundGray = "#27272fff",
            Surface = "#373740ff",
            DrawerBackground = "#373740ff",
            DrawerText = "#ffffff7f",
            DrawerIcon = "#ffffff7f",
            AppbarBackground = "#27272fff",
            AppbarText = "rgba(255,255,255,0.6980392156862745)",
            LinesDefault = "rgba(255,255,255,0.11764705882352941)",
            LinesInputs = "rgba(255,255,255,0.2980392156862745)",
            TableLines = "rgba(255,255,255,0.11764705882352941)",
            TableStriped = "rgba(255,255,255,0.2)",
            Divider = "rgba(255,255,255,0.11764705882352941)",
            DividerLight = "rgba(255,255,255,0.058823529411764705)",
            PrimaryDarken = "#cc7800ff",
            PrimaryLighten = "#ffb74dff",
            SecondaryDarken = "#e0e0e0ff",
            SecondaryLighten = "#fae6e6ff",
            InfoDarken = "rgb(10,133,255)",
            InfoLighten = "rgb(92,173,255)",
            SuccessDarken = "rgb(9,154,108)",
            SuccessLighten = "rgb(13,222,156)",
            WarningDarken = "rgb(214,143,0)",
            WarningLighten = "rgb(255,182,36)",
            ErrorDarken = "rgb(244,47,70)",
            ErrorLighten = "rgb(248,119,134)",
            DarkDarken = "#17171cff",
            DarkLighten = "#383843ff",
        },
        LayoutProperties = new LayoutProperties()
        {
            DefaultBorderRadius = "4px",
            DrawerMiniWidthLeft = "56px",
            DrawerMiniWidthRight = "56px",
            DrawerWidthLeft = "240px",
            DrawerWidthRight = "240px",
            AppbarHeight = "64px",
        },
        Typography = new Typography()
        {
            Default = new DefaultTypography
            {
                FontFamily = [ "Roboto", "Inter", "Segoe UI", "Arial", "sans-serif" ],
                FontWeight = "400",
                FontSize = ".875rem",
                LineHeight = "1.43",
                LetterSpacing = ".01071em",
                TextTransform = "none",
            },
            H1 = new H1Typography
            {
                FontWeight = "300",
                FontSize = "3.25rem",
                LineHeight = "1.2",
                LetterSpacing = "-.01em",
                TextTransform = "none",
            },
            H2 = new H2Typography
            {
                FontWeight = "400",
                FontSize = "2.5rem",
                LineHeight = "1.3",
                LetterSpacing = "-.005em",
                TextTransform = "none",
            },
            H3 = new H3Typography
            {
                FontWeight = "500",
                FontSize = "2rem",
                LineHeight = "1.4",
                LetterSpacing = "0",
                TextTransform = "none",
            },
            H4 = new H4Typography
            {
                FontWeight = "500",
                FontSize = "1.5rem",
                LineHeight = "1.4",
                LetterSpacing = ".007em",
                TextTransform = "none",
            },
            H5 = new H5Typography
            {
                FontWeight = "600",
                FontSize = "1.25rem",
                LineHeight = "1.5",
                LetterSpacing = ".01em",
                TextTransform = "none",
            },
            H6 = new H6Typography
            {
                FontWeight = "600",
                FontSize = "1.125rem",
                LineHeight = "1.6",
                LetterSpacing = ".001em",
                TextTransform = "none",
            },
            Subtitle1 = new Subtitle1Typography
            {
                FontWeight = "400",
                FontSize = "1.0625rem",
                LineHeight = "1.6",
                LetterSpacing = ".005em",
                TextTransform = "none",
            },
            Subtitle2 = new Subtitle2Typography
            {
                FontWeight = "500",
                FontSize = ".9375rem",
                LineHeight = "1.5",
                LetterSpacing = ".005em",
                TextTransform = "none",
            },
            Body1 = new Body1Typography
            {
                FontWeight = "400",
                FontSize = "1rem",
                LineHeight = "1.5",
                LetterSpacing = ".001em",
                TextTransform = "none",
            },
            Body2 = new Body2Typography
            {
                FontWeight = "400",
                FontSize = ".875rem",
                LineHeight = "1.4",
                LetterSpacing = ".01em",
                TextTransform = "none",
            },
            Button = new ButtonTypography
            {
                FontWeight = "500",
                FontSize = ".875rem",
                LineHeight = "1.5",
                LetterSpacing = ".02em",
                TextTransform = "none"
            },
            Caption = new CaptionTypography
            {
                FontWeight = "400",
                FontSize = ".75rem",
                LineHeight = "1.6",
                LetterSpacing = ".02em",
                TextTransform = "none",
            },
            Overline = new OverlineTypography
            {
                FontWeight = "400",
                FontSize = ".75rem",
                LineHeight = "1.8",
                LetterSpacing = ".05em",
                TextTransform = "uppercase",
            },
        },
        ZIndex = new ZIndex()
        {
            Drawer = 1100,
            Popover = 1200,
            AppBar = 1300,
            Dialog = 1400,
            Snackbar = 1500,
            Tooltip = 1600,
        },
    };
}
